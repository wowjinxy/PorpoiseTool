"""
Handler for PowerPC fmuls (Floating-Point Multiply Single) instruction.
"""
...
class FmulsHandler:
    """Handles PowerPC fmuls instruction transpilation."""
    ...
    def handle(self, instruction: Instruction) -> List[str]:
        ...
        try:
            ...
            if opcode == 'fmuls':
                return [
                    f"gc_env.f[{dst_fpreg}] = (float)gc_env.f[{src_a_fpreg}] * (float)gc_env.f[{src_b_fpreg}]; // fmuls f{dst_fpreg}, f{src_a_fpreg}, f{src_b_fpreg}"
                ]
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for fmuls instruction handling."""
    return FmulsHandler(transpiler).handle(instruction)