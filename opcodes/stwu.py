"""
Handler for PowerPC stwu (Store Word with Update) instruction.
"""
...
class StwuHandler:
    """Handles PowerPC stwu instruction transpilation."""
    ...
    def parse_memory_operand(self, mem_str: str) -> Tuple[Union[str, int], int, bool, int]:
        """Parse memory operand like 'offset(rN)', 'symbol@l(rN)', or 'symbol@sda21(rN)'."""
        mem_str = mem_str.strip()

        # Handle SDA relocation format
        if '@sda21(' in mem_str:
    ...
    def generate_memory_access(self, opcode: str, src_reg: int, offset_or_symbol: Union[str, int], base_reg: int, is_sda: bool, addend: int = 0) -> List[str]:
        """Generate memory access code for stwu."""
        lines: List[str] = []
        if is_sda:
            ...
            if base_reg == 0:
                lines.append(f"gc_mem_write32(gc_env.ram, {addr}, gc_env.r[{src_reg}]); // {opcode} r{src_reg}, {symbol}@sda21(r0)")
            else:
                lines.append(f"gc_mem_write32(gc_env.ram, gc_env.r[{base_reg}] + {addr}, gc_env.r[{src_reg}]); // {opcode} r{src_reg}, {symbol}@sda21(r{base_reg})")
            lines.append(f"gc_env.r[{base_reg}] = gc_env.r[{base_reg}] + {addr};")
        elif isinstance(offset_or_symbol, str):
            symbol = offset_or_symbol
            if base_reg == 0:
                lines.append(f"gc_mem_write32(gc_env.ram, (uint32_t)&{symbol}, gc_env.r[{src_reg}]); // {opcode} r{src_reg}, {symbol}@l/h(r0)")
            else:
                lines.append(f"gc_mem_write32(gc_env.ram, gc_env.r[{base_reg}] + (uint32_t)&{symbol}, gc_env.r[{src_reg}]); // {opcode} r{src_reg}, {symbol}@l/h(r{base_reg})")
            lines.append(f"gc_env.r[{base_reg}] = gc_env.r[{base_reg}] + (uint32_t)&{symbol};")
        else:
            offset = offset_or_symbol
            if offset == 0:
                lines.append(f"gc_mem_write32(gc_env.ram, gc_env.r[{base_reg}], gc_env.r[{src_reg}]); // {opcode} r{src_reg}, (r{base_reg})")
            else:
                offset_hex = format_hex(offset)
                lines.append(f"gc_mem_write32(gc_env.ram, gc_env.r[{base_reg}] + {offset_hex}, gc_env.r[{src_reg}]); // {opcode} r{src_reg}, {offset_hex}(r{base_reg})")
            inc = '0' if offset == 0 else format_hex(offset)
            lines.append(f"gc_env.r[{base_reg}] = gc_env.r[{base_reg}] + {inc};")
        return lines

    def handle(self, instruction: Instruction) -> List[str]:
        """Handle stwu instruction."""
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for stwu instruction handling."""
    return StwuHandler(transpiler).handle(instruction)
