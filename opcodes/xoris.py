"""
Handler for PowerPC xoris (XOR Immediate Shifted) instruction.
"""
...
class XorisHandler:
    """Handles PowerPC xoris instruction transpilation."""
    ...
    def handle(self, instruction: Instruction) -> List[str]:
        ...
        try:
            ...
            if opcode == 'xoris':
                shifted_imm = imm << 16
                return [
                    f"gc_env.r[{dst_reg}] = gc_env.r[{src_reg}] ^ 0x{shifted_imm:X}; // xoris r{dst_reg}, r{src_reg}, 0x{imm:X}"
                ]
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for xoris instruction handling."""
    return XorisHandler(transpiler).handle(instruction)