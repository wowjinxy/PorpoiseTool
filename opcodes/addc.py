...
class AddcHandler:
    ...
    def handle(self, instruction: Instruction) -> List[str]:
        ...
        try:
            ...
            result = [
                f"uint64_t addc_temp = (uint64_t)gc_env.r[{src_reg1}] + gc_env.r[{src_reg2}];",
                f"gc_env.r[{dst_reg}] = (uint32_t)addc_temp; // {opcode} r{dst_reg}, r{src_reg1}, r{src_reg2}",
                f"gc_env.xer = (gc_env.xer & ~0x20000000) | (addc_temp > 0xFFFFFFFF ? 0x20000000 : 0);",
            ]
            return result
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    ...
    return AddcHandler().handle(instruction)
