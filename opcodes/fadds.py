"""
Handler for PowerPC fadds (Floating-Point Add Single) instruction.
"""
...
class FaddsHandler:
    """Handles PowerPC fadds instruction transpilation."""
    ...
    def handle(self, instruction: Instruction) -> List[str]:
        ...
        try:
            ...
            if opcode == 'fadds':
                return [
                    f"gc_env.f[{dst_fpreg}] = (float)gc_env.f[{src_a_fpreg}] + (float)gc_env.f[{src_b_fpreg}]; // fadds f{dst_fpreg}, f{src_a_fpreg}, f{src_b_fpreg}"
                ]
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for fadds instruction handling."""
    return FaddsHandler(transpiler).handle(instruction)