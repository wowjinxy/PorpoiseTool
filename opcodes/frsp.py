"""
Handler for PowerPC frsp (Floating-Point Round to Single Precision) instruction.
"""
...
class FrspHandler:
    """Handles PowerPC frsp instruction transpilation."""
    ...
    def handle(self, instruction: Instruction) -> List[str]:
        ...
        try:
            ...
            if opcode == 'frsp':
                return [
                    f"gc_env.f[{dst_fpreg}] = (float)gc_env.f[{src_fpreg}]; // frsp f{dst_fpreg}, f{src_fpreg}"
                ]
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for frsp instruction handling."""
    return FrspHandler(transpiler).handle(instruction)