"""
Handler for PowerPC fmsub (Floating-Point Multiply-Subtract) instruction.
"""
...
class FmsubHandler:
    """Handles PowerPC fmsub instruction transpilation."""
    ...
    def handle(self, instruction: Instruction) -> List[str]:
        ...
        try:
            ...
            if opcode == 'fmsub':
                return [
                    f"gc_env.f[{dst_fpreg}] = gc_env.f[{src_a_fpreg}] * gc_env.f[{src_b_fpreg}] - gc_env.f[{src_c_fpreg}]; // fmsub f{dst_fpreg}, f{src_a_fpreg}, f{src_b_fpreg}, f{src_c_fpreg}"
                ]
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for fmsub instruction handling."""
    return FmsubHandler(transpiler).handle(instruction)