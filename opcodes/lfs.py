"""
Handler for PowerPC lfs (Load Floating-Point Single) instruction.
"""
...
class LfsHandler:
    """Handles PowerPC lfs instruction transpilation."""
        ...
        try:
            ...
            if '@sda21(' in ops[1]:
                ...
                if len(sda_match) == 2:
                    ...
                    self.transpiler.variables.add(f"extern float {symbol}")
                    
                    if base_reg == 0:  # r0 means absolute SDA access
                        return [f"gc_env.f[{dst_fpreg}] = {symbol}; // lfs f{dst_fpreg}, {symbol}@sda21(r0)"]
                    else:
                        return [f"gc_env.f[{dst_fpreg}] = *((float*)(gc_env.r[{base_reg}] + (uint32_t)&{symbol})); // lfs f{dst_fpreg}, {symbol}@sda21(r{base_reg})"]
            ...
            raise ValueError(f"Unrecognized lfs operand format: {ops[1]}")
            
        except ValueError as e:
            return [f"// Error processing {opcode} {' '.join(ops)}: {str(e)}"]

def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for lfs instruction handling."""
    return LfsHandler(transpiler).handle(instruction, transpiler)