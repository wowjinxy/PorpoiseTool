"""
Handler for PowerPC fnmsub (Floating-Point Negative Multiply-Subtract) instruction.
"""
...
class FnmsubHandler:
    """Handles PowerPC fnmsub instruction transpilation."""
    ...
    def handle(self, instruction: Instruction) -> List[str]:
        ...
        try:
            ...
            if opcode == 'fnmsub':
                return [
                    f"gc_env.f[{dst_fpreg}] = -(gc_env.f[{src_a_fpreg}] * gc_env.f[{src_b_fpreg}] - gc_env.f[{src_c_fpreg}]); // fnmsub f{dst_fpreg}, f{src_a_fpreg}, f{src_b_fpreg}, f{src_c_fpreg}"
                ]
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for fnmsub instruction handling."""
    return FnmsubHandler(transpiler).handle(instruction)