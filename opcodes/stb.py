"""
Handler for PowerPC stb (Store Byte) instruction.
"""
...
class StbHandler:
    """Handles PowerPC stb instruction transpilation."""
    ...
    def parse_memory_operand(self, mem_str: str, transpiler) -> Tuple[Union[str, int], int, bool, int]:
        ...
        if '@sda21(' in mem_str:
            ...
            if len(parts) == 2:
                ...
                transpiler.variables.add(f"extern uint8_t {symbol}")
        ...
        if '@h(' in mem_str:
            ...
            if len(h_match) == 2:
                symbol = transpiler.sanitize_symbol_name(h_match[0])
        ...
        if mem_str.startswith('sda:'):
            symbol = transpiler.sanitize_symbol_name(mem_str[4:].strip())
            transpiler.variables.add(f"extern uint8_t {symbol}")
    ...
    def generate_memory_access(self, opcode: str, reg: int, off_or_sym: Union[str, int], base_reg: int, is_sda: bool, addend: int = 0) -> List[str]:
        """Generate memory access code for stb."""
        if is_sda:
            symbol = off_or_sym
            ...
            if base_reg == 0:
                return [f"gc_mem_write8(gc_env.ram, {addr}, gc_env.r[{reg}]); // {opcode} r{reg}, {symbol}@sda21(r0)"]
            else:
                return [f"gc_mem_write8(gc_env.ram, gc_env.r[{base_reg}] + {addr}, gc_env.r[{reg}]); // {opcode} r{reg}, {symbol}@sda21(r{base_reg})"]
        elif isinstance(off_or_sym, str):
            symbol = off_or_sym
            if base_reg == 0:
                return [f"gc_mem_write8(gc_env.ram, (uint32_t)&{symbol}, gc_env.r[{reg}]); // {opcode} r{reg}, {symbol}@l/h(r0)"]
            else:
                return [f"gc_mem_write8(gc_env.ram, gc_env.r[{base_reg}] + (uint32_t)&{symbol}, gc_env.r[{reg}]); // {opcode} r{reg}, {symbol}@l/h(r{base_reg})"]
        else:
            offset = off_or_sym
            if offset == 0:
                return [f"gc_mem_write8(gc_env.ram, gc_env.r[{base_reg}], gc_env.r[{reg}]); // {opcode} r{reg}, (r{base_reg})"]
            else:
                return [f"gc_mem_write8(gc_env.ram, gc_env.r[{base_reg}] + {format_hex(offset)}, gc_env.r[{reg}]); // {opcode} r{reg}, {format_hex(offset)}(r{base_reg})"]

    def handle(self, instruction: Instruction, transpiler) -> List[str]:
        """Handle stb instruction."""
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for stb instruction handling."""
    return StbHandler().handle(instruction, transpiler)
