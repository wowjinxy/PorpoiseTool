"""
Handler for PowerPC slwi (Shift Left Word Immediate) instruction.
"""
...
class SlwiHandler:
    """Handles PowerPC slwi instruction transpilation."""
    ...
    def handle(self, instruction: Instruction) -> List[str]:
        ...
        try:
            ...
            if opcode == 'slwi':
                return [f"gc_env.r[{dst_reg}] = gc_env.r[{src_reg}] << {shift}; // slwi r{dst_reg}, r{src_reg}, {shift}"]
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for slwi instruction handling."""
    return SlwiHandler(transpiler).handle(instruction)