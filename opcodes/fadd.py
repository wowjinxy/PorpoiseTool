"""
Handler for PowerPC fadd (Floating-Point Add) instruction.
"""
...
class FaddHandler:
    """Handles PowerPC fadd instruction transpilation."""
    ...
    def handle(self, instruction: Instruction) -> List[str]:
        ...
        try:
            ...
            if opcode == 'fadd':
                return [
                    f"gc_env.f[{dst_fpreg}] = gc_env.f[{src_a_fpreg}] + gc_env.f[{src_b_fpreg}]; // fadd f{dst_fpreg}, f{src_a_fpreg}, f{src_b_fpreg}"
                ]
...
def handle(instruction: Instruction, transpiler: 'ModularTranspiler') -> List[str]:
    """Entry point for fadd instruction handling."""
    return FaddHandler(transpiler).handle(instruction)